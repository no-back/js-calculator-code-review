import { MESSAGE } from "../../src/js/utils/const";
...
describe("Calculator Alert Test", () => {
  ...
  it("두번째 숫자를 입력하지 않고 '=' 연산자를 입력할 경우 경고 메시지를 띄운다.", () => {
    ...
    cy.get(".operation")
      ...
      .then(() => {
        expect(alertStub.getCall(0)).to.be.calledWith(
          MESSAGE.NEED_SECOND_NUMBER_MESSAGE
        );
      });
  });
  ...
  it("연산자 입력 경고 메시지를 확인한다.", () => {
    ...
    cy.get(".operation")
      ...
      .then(() => {
        ...
        expect(actualMessage).to.equal(MESSAGE.NEED_SECOND_NUMBER_MESSAGE);
      });
  });
  ...
  it("숫자를 4자리 이상 입력할 경우 경고 메시지를 띄운다.", () => {
    ...
    cy.get(".digit")
      ...
      .then(() => {
        expect(alertStub.getCall(0)).to.be.calledWith(
          MESSAGE.LIMITED_NUMBER_WARNING_MESSAGE
        );
      });
  });
  ...
  it("숫자 4자리 이상 입력 경고 메시지를 확인한다.", () => {
    ...
    cy.get(".digit")
      ...
      .then(() => {
        const actualMessage = alertStub.getCall(0).lastArg;
        expect(actualMessage).to.be.equal(
          MESSAGE.LIMITED_NUMBER_WARNING_MESSAGE
        );
      });
  });
